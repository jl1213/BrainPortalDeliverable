...
{
    public class V3MCQ : MonoBehaviour
    {
        ...
        public Animator brainAnimator; // Reference to Animator for brain animations
        ...
        public class Question
        {
            ...
            public string animationName; // Stores animation name from CSV

            public Question(string question, List<string> options, string correctAnswer, string animationName)
            {
        ...
        void Start()
        {
            ...
            {
                Debug.LogError("CSV file not assigned! Please drag and drop a CSV file into the V3MCQ component.");
            }
        }
        ...
        public List<Question> ReadCSVFile(TextAsset file)
        {
            ...
            foreach (string line in lines)
            {
                ...
                if (values.Length >= 8) // Ensure at least 8 columns exist (including animation name)
                {
                    string question = values[0].Trim();
                    string correctAnswer = values[1].Trim();
                    string animationName = values[7].Trim();
                    ...
                    ShuffleList(options);
                    questionList.Add(new Question(question, options, correctAnswer, animationName));
                }
            }
            return questionList;
        }
